...
export function formatSearchResults(results, query = '', limit = MAX_RESULTS_DISPLAY, offset = 0) {
  ...
  let message = `${t('search.results.title', query)}\n\n`;
  displayed.forEach((media, idx) => {
    ...
    const typeEmoji = typeStr === 'Movie' ? t('media.movie') : t('media.tv');
  ...
  if (showMoreOption) {
    ...
    message += `\n${showMoreEmoji} ${t('search.results.showMore')}\n`;
  }
  
  message += `\n0️⃣ ${t('search.results.cancel')}\n`;
  message += `\n📥 ${t('search.results.reply')}`;
