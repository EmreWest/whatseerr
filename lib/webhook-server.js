...
import { getUserIdFromEmail, getConfigPath, getPhoneNumberFromUserId, isLidFormat } from './utils.js';
...
async function handleReaction(cfg, jellyseerrClient, wahaClient, webhookData) {
  ...
  logger?.debug('Reaction received', { messageId, reactionText, from: payload.from });
  ...
  if (messageIdParts.length < 3) {
    logger?.debug('Invalid messageId format, cannot extract chatId', { messageId });
    return;
  }
  ...
  if (!message || !message.body) {
    logger?.debug('Could not fetch message or message has no body', { messageId, chatId });
    return;
  }
  ...
  if (!adminPhoneNumber) {
    logger?.warn('Admin phone number not configured, cannot validate reaction', { messageId, requestId });
    return;
  }
  ...
  let isAdminReaction = false;
  
  // Direct match (phone format)
  if (reactorChatId === expectedAdminPhoneChatId) {
    isAdminReaction = true;
  } else if (isLidFormat(reactorChatId)) {
    // If reactor is LID format, resolve it and compare with admin phone
    const reactorPhone = await getPhoneNumberByLid(wahaClient, cfg, reactorChatId);
    if (reactorPhone === expectedAdminPhoneChatId) {
      isAdminReaction = true;
    }
  }
  
  if (!isAdminReaction) {
    logger?.warn('Reaction not from admin, ignoring', {
      messageId,
      reactorChatId,
  ...
  logger?.info(`Reaction detected on approval message from admin`, {
    messageId,
    requestId,
    reactionText,
    from: payload.from
  ...
  if (!isApprove && !isDecline) {
    logger?.debug('Reaction is not approve or decline emoji, ignoring', { reactionText });
    return;
  }
  
  const adminChatId = expectedAdminPhoneChatId;
  
  try {
    // sendMessage handles chatId format conversion internally
    if (isApprove) {
      ...
      await sendMessage(wahaClient, cfg, adminChatId, 
    ...
    // Clean up: remove from pendingRequestApprovals if exists
    pendingRequestApprovals.delete(adminChatId);
    ...
    try {
      ...
      logger?.error('Failed to send error message', sendErr?.message || sendErr);
    }
  }
}
      ...
      try {
        ...
        if (webhookData.event === 'message.reaction' && webhookData.payload) {
          handleReaction(cfg, jellyseerrClient, wahaClient, webhookData).catch((err) => {
            logger?.error('Error in handleReaction', err?.message || err);
            if (err?.stack) {
              logger?.debug('handleReaction stack', err.stack);
            }
          });
        }
