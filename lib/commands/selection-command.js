  ...
  async execute(context) {
    ...
    if (hasPendingTv) {
      try {
        ...
        if (!tvShow) {
          ...
          stateManager.clearUserFlow(chatId);
          try {
            await sendMessage(wahaClient, cfg, chatId, `${t('request.error', 'Invalid selection. Please search again')}`);
          } catch (err) {
        ...
        stateManager.clearUserFlow(chatId);
        try {
          await sendMessage(wahaClient, cfg, chatId, `${t('request.error', 'An error occurred. Please try again')}`);
        } catch (sendErr) {
    ...
    try {
      ...
      if (!results || !Array.isArray(results) || results.length === 0) {
        ...
        stateManager.clearUserFlow(chatId);
        try {
          await sendMessage(wahaClient, cfg, chatId, `${t('request.error', 'No results available. Please search again')}`);
        } catch (err) {
