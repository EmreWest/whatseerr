  ...
  async execute(context) {
    ...
    if (hasPendingTv) {
      try {
        ...
        if (!tvShow) {
          ...
          stateManager.clearUserFlow(chatId);
          try {
            await sendMessage(wahaClient, cfg, chatId, `❌ ${t('selection.invalid')}`);
          } catch (err) {
        ...
        stateManager.clearUserFlow(chatId);
        try {
          await sendMessage(wahaClient, cfg, chatId, `❌ ${t('request.error')}`);
        } catch (sendErr) {
    ...
    try {
      ...
      if (!results || !Array.isArray(results) || results.length === 0) {
        ...
        stateManager.clearUserFlow(chatId);
        try {
          await sendMessage(wahaClient, cfg, chatId, `❌ ${t('selection.no_results')}`);
        } catch (err) {
      ...
      if (selectionNumber >= 1 && selectionNumber <= displayedCount) {
        ...
        try {
          await sendMessage(wahaClient, cfg, chatId, `❌ ${t('selection.invalid_range', maxOption)}`);
        } catch (err) {
