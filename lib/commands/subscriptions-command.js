...
export class SubscriptionsCommand extends BaseCommand {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Fetch media details for each subscription and clean up available ones
      ...
      for (const sub of subscriptions) {
        try {
          ...
          if (statusInfo && isAvailable(statusInfo.status)) {
            // Media is available, remove subscription
            ...
            continue; // Skip adding to subscriptionDetails
          }
          ...
          // Include with minimal info if fetch fails (don't remove on error)
          subscriptionDetails.push({
            ...sub,
      ...
      if (subscriptionsToRemove.length > 0) {
        ...
        logger?.info('Cleaned up available subscriptions', {
          chatId,
          removedCount: subscriptionsToRemove.length
        });
      }
      
      // Check if any subscriptions remain after cleanup
