...
import { getMediaDetails, formatMedia, extractMediaStatus, formatQualityText } from '../request.js';
  ...
  async execute(context) {
    ...
    try {
      ...
      for (const sub of subscriptions) {
        try {
          ...
          if (statusInfo && isAvailable(statusInfo.status)) {
            ...
            logger?.info(`🧹 Removing subscription: "${formatted.title}"${formatQualityText(sub.is4k)} (now available)`);
            continue;
          }
      ...
      if (subscriptionsToRemove.length > 0) {
        logger?.info(`🧹 Cleaned up ${subscriptionsToRemove.length} subscription${subscriptionsToRemove.length !== 1 ? 's' : ''} (media now available)`);
      }
