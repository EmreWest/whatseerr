...
export class SearchCommand extends BaseCommand {
  ...
  async execute(context) {
    ...
    if (!stateManager.tryAcquireFlowLock(chatId)) {
      logger?.info('User has active flow, command will wait in queue', { chatId, query });
    ...
    try {
      ...
      // Store results in state ONLY AFTER successfully sending message to user.
      // This prevents race condition where user sends selection number before seeing the results message.
