...
import { STATUS_AVAILABLE, STATUS_PROCESSING, STATUS_PARTIALLY_AVAILABLE } from './constants.js';
...
export function convertRequestedState(status) {
  ...
  if (status === STATUS_PROCESSING || status === STATUS_PARTIALLY_AVAILABLE || status === STATUS_AVAILABLE) {
...
export function checkSeasonRequestStatus(mediaDetails, seasonNumber, is4k = false) {
  ...
  if (mediaDetails.seasons && Array.isArray(mediaDetails.seasons)) {
    ...
    if (seasonMeta) {
      ...
      if (status !== undefined && status !== null) {
        ...
        isAvailable = status === STATUS_AVAILABLE; // Only status 5 is available
  ...
  if (mediaInfo.requests && Array.isArray(mediaInfo.requests)) {
    const pendingRequest = mediaInfo.requests.find(req => {
      ...
      // req.is4k can be true, false, or undefined
  ...
  if (!hasPendingRequest) {
    // Check lowercase first (actual API format), then uppercase as fallback
 ...
 * @param {number[]|"all"|null} requestedSeasons - Array of requested season numbers or string "all" (optional, for TV shows)
...
export function getRequestStatusMessage(res, typeStr, isTvShow = false, is4k = false, media = null, requestedSeasons = null) {
  ...
  if (res.status === 201 || res.status === 200) {
    ...
    if (isTvShow && mediaTitle) {
      // Handle "all" string or array of season numbers per API spec
      // API reference: seasons: oneOf: [array of numbers, string enum: ["all"]]
      if (requestedSeasons === 'all') {
        // All seasons requested as string "all"
        return getAllSeasonsRequestSuccessMessage(mediaTitle);
      } else if (Array.isArray(requestedSeasons) && requestedSeasons.length > 0) {
        ...
        // All seasons requested (no specific seasons array or null)
        return getAllSeasonsRequestSuccessMessage(mediaTitle);
      }
    } else if (mediaTitle) {
  ...
  if (res.status === 202) {
    ...
    if (isTvShow && mediaTitle) {
      // Handle "all" string or array of season numbers
      if (requestedSeasons === 'all') {
        return getAllSeasonsRequestSuccessMessage(mediaTitle);
      } else if (Array.isArray(requestedSeasons) && requestedSeasons.length === 1) {
        return getSeasonRequestSuccessMessage(requestedSeasons[0], mediaTitle);
      }
      return getAllSeasonsRequestSuccessMessage(mediaTitle);
    } else if (mediaTitle) {
