...
import { extractMediaStatus, formatStatusMessage } from './request.js';
 ...
 */
export function getRequestStatusMessage(res, typeStr, isTvShow = false, is4k = false) {
  
  if (res.status === 201 || res.status === 200) {
    // For successful request creation, just confirm it was created
    // Don't check status - we just created it, so it's obviously new
