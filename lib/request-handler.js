...
async function subscribeToNotificationsIfNotRequester(cfg, wahaClient, chatId, mediaDetails, mediaId, mediaType, mediaTitle, is4k, logger, checkWasAlreadyRequester, logContext = 'new request') {
  ...
  if (!wasAlreadyRequester) {
    try {
      ...
      subscriptionManager.addSubscription(mediaId, mediaType, is4k, chatId, mediaTitle);
      const mediaTypeName = mediaType === 1 ? 'movie' : 'TV show';
      logger?.info(`Subscribed user to ${mediaTypeName} notifications (${logContext})`, {
        mediaId,
        is4k,
        chatId
      });
    ...
    logger?.info('Skipping subscription - user is already the requester (Seerr handles notifications)', {
      mediaId,
      userId,
      is4k,
      chatId
