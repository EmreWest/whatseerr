...
import { getMediaDetails, createRequest, formatMedia, extractMediaStatus, formatStatusMessage } from './request.js';
...
export async function handleTvSeasonSelection(cfg, jellyseerrClient, wahaClient, chatId, messageText, tvShow, logger, is4k = false) {
  ...
  if (seasonSelection.cancelled) {
    ...
    try {
      await sendMessage(wahaClient, cfg, chatId, '❌ Season selection cancelled');
    } catch (err) {
  ...
  if (mediaDetails) {
    ...
    if (canRequestSeasons.length === 0 && (alreadyRequestedSeasons.length > 0 || alreadyAvailableSeasons.length > 0)) {
      ...
      if (alreadyRequestedSeasons.length > 0 && !userRequestInfo.hasRequestedAny) {
        try {
          const subscriptionManager = getSubscriptionManager();
          const mediaType = 2; // 2 = tv
          const { title } = formatMedia(tvShow);
          subscriptionManager.addSubscription(tvShow.id, mediaType, is4k, chatId, title);
          logger?.info('Subscribed user to TV show notifications (seasons already requested by someone else)', { 
            mediaId: tvShow.id, 
            is4k, 
            chatId,
            requestedSeasons: alreadyRequestedSeasons
          });
        } catch (err) {
          logger?.warn('Failed to subscribe user to notifications for already requested TV seasons', {
            ...getErrorDetails(err, 'subscribeTvNotifications'),
            mediaId: tvShow.id,
            chatId
          });
        }
      } else if (userRequestInfo.hasRequestedAny) {
        logger?.info('Skipping subscription - user is the original requester for these seasons (Seerr handles notifications)', {
          mediaId: tvShow.id,
          userId,
          requestedSeasons: userRequestInfo.requestedSeasons,
          is4k,
          chatId
        });
      }
    ...
    if (alreadyRequestedSeasons.length > 0 || alreadyAvailableSeasons.length > 0) {
      ...
      if (alreadyRequestedSeasons.length > 0 && !userRequestInfo.hasRequestedAny) {
        try {
          ...
          const { title } = formatMedia(tvShow);
          subscriptionManager.addSubscription(tvShow.id, mediaType, is4k, chatId, title);
          logger?.info('Subscribed user to TV show notifications (some seasons already requested by someone else)', { 
            mediaId: tvShow.id, 
            is4k, 
            chatId,
            requestedSeasons: alreadyRequestedSeasons
          ...
          logger?.warn('Failed to subscribe user to notifications for partially requested TV seasons', {
            ...getErrorDetails(err, 'subscribeTvNotifications'),
            mediaId: tvShow.id,
            chatId
          });
        ...
        logger?.info('Skipping subscription - user is the original requester for some seasons (Seerr handles notifications)', {
          mediaId: tvShow.id,
          userId,
          requestedSeasons: userRequestInfo.requestedSeasons,
          is4k,
  ...
  try {
      ...
      if (res.status === 201 || res.status === 200) {
        // Check if user was already a requester for any of these seasons before this request
        const userId = await getUserIdFromChatId(cfg, chatId, wahaClient);
        // Convert "all" to array of all seasons for checking user requests
        let seasonsForCheck = seasons;
        if (seasons === 'all' && mediaDetails) {
          const allSeasons = mediaDetails.seasons || mediaDetails.Seasons || [];
        ...
        // Only subscribe if user was NOT already a requester for these seasons (Seerr handles notifications for original requester)
        // Note: This check uses the old mediaDetails, so if user requests new seasons, they weren't a requester for those before
        const wasAlreadyRequester = userRequestInfo.hasRequestedAny;
        ...
        if (!wasAlreadyRequester) {
          try {
            ...
            const { title } = formatMedia(tvShow);
            subscriptionManager.addSubscription(tvShow.id, mediaType, is4k, chatId, title);
            logger?.debug('Subscribed user to TV show notifications (new request)', { 
              mediaId: tvShow.id, 
              is4k, 
              chatId 
            });
          ...
          logger?.debug('Skipping subscription - user was already a requester for these seasons (Seerr handles notifications)', {
            mediaId: tvShow.id,
            userId,
            requestedSeasons: userRequestInfo.requestedSeasons,
            is4k,
...
export async function handleTvShowSelection(cfg, jellyseerrClient, wahaClient, chatId, chosen, logger, is4k = false) {
  ...
  try {
    ...
    if (seasons.length === 0) {
        ...
        if (available) {
          ...
          if (!wasAlreadyRequester) {
            try {
              ...
              subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId, title);
              logger?.info('Subscribed user to TV show notifications (already requested by someone else, no season info)', { 
                mediaId: chosen.id, 
                is4k, 
                chatId 
              });
            } catch (err) {
              logger?.warn('Failed to subscribe user to notifications for already requested TV show', {
                ...getErrorDetails(err, 'subscribeTvShowNotifications'),
                mediaId: chosen.id,
                chatId
              });
            }
          } else {
            logger?.info('Skipping subscription - user is the original requester (Seerr handles notifications)', {
              mediaId: chosen.id,
              userId,
              is4k,
              chatId
      ...
      // Only subscribe if this is a NEW request (not a duplicate), as Seerr handles notifications for original requesters
      if (res.status === 201 || res.status === 200) {
        // Check if user was already a requester before this request
        const userId = await getUserIdFromChatId(cfg, chatId, wahaClient);
        const wasAlreadyRequester = checkUserRequestedTvShow(mediaDetails, userId, is4k);
        
        // Only subscribe if user was NOT already a requester (Seerr handles notifications for original requester)
        if (!wasAlreadyRequester) {
          try {
            const subscriptionManager = getSubscriptionManager();
            const mediaType = 2; // 2 = tv
            const { title } = formatMedia(chosen);
            subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId, title);
            logger?.debug('Subscribed user to TV show notifications (new request, all seasons)', { 
              mediaId: chosen.id, 
              is4k, 
              chatId 
            });
          ...
          logger?.debug('Skipping subscription - user is already a requester (Seerr handles notifications)', {
            mediaId: chosen.id,
            userId,
            is4k,
            chatId
    ...
    if (canRequestSeasons.length === 0 && requestedSeasons.length > 0) {
      ...
      if (!allAvailable && !wasAlreadyRequester) {
        try {
          const subscriptionManager = getSubscriptionManager();
          const mediaType = 2; // 2 = tv
          const { title } = formatMedia(chosen);
          subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId, title);
          logger?.info('Subscribed user to TV show notifications (all seasons already requested by someone else)', { 
            mediaId: chosen.id, 
            is4k, 
            chatId 
          });
...
export async function handleMovieSelection(cfg, jellyseerrClient, wahaClient, chatId, chosen, logger, is4k = false) {
  ...
  try {
        ...
        if (available) {
          ...
          if (mediaDetails?.mediaInfo?.requests && Array.isArray(mediaDetails.mediaInfo.requests)) {
            ...
            if (userRequest) {
              ...
              logger?.info('User is the original requester', { 
                mediaId: chosen.id, 
                userId, 
                requestDate,
                is4k
          ...
          if (!isSelfRequested) {
            try {
              const subscriptionManager = getSubscriptionManager();
              const mediaType = 1; // 1 = movie
              const { title } = formatMedia(chosen);
              subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId, title);
              logger?.info('Subscribed user to movie notifications (already requested by someone else)', { 
                mediaId: chosen.id, 
                is4k, 
                chatId 
              });
            } catch (err) {
              logger?.warn('Failed to subscribe user to notifications for already requested movie', {
                ...getErrorDetails(err, 'subscribeMovieNotifications'),
                mediaId: chosen.id,
                chatId
              });
            }
    ...
    // Only subscribe if this is a NEW request (not a duplicate), as Seerr handles notifications for original requesters
    if (res.status === 201 || res.status === 200) {
      // Check if user was already the requester before this request
      const userId = await getUserIdFromChatId(cfg, chatId, wahaClient);
      let wasAlreadyRequester = false;
      
      if (mediaDetails?.mediaInfo?.requests && Array.isArray(mediaDetails.mediaInfo.requests)) {
        wasAlreadyRequester = mediaDetails.mediaInfo.requests.some(req => {
          const requestUserId = req.requestedBy?.id;
          const requestIs4k = req.is4k === true;
          return requestUserId === userId && requestIs4k === is4k;
        });
      ...
      if (!wasAlreadyRequester) {
        try {
          ...
          const { title } = formatMedia(chosen);
          subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId, title);
          logger?.debug('Subscribed user to movie notifications (new request)', { 
            mediaId: chosen.id, 
            is4k, 
            chatId 
          });
        ...
        logger?.debug('Skipping subscription - user is already the requester (Seerr handles notifications)', {
          mediaId: chosen.id,
          userId,
          is4k,
          chatId
