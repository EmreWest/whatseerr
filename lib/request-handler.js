...
import { getMediaDetails, createRequest, formatMedia, extractMediaStatus, formatStatusMessage, formatQualityText } from './request.js';
...
async function subscribeToNotificationsIfNotRequester(cfg, wahaClient, chatId, mediaDetails, mediaId, mediaType, mediaTitle, is4k, logger, checkWasAlreadyRequester, logContext = 'new request') {
  ...
  if (!wasAlreadyRequester) {
    try {
      ...
      subscriptionManager.addSubscription(mediaId, mediaType, is4k, chatId, formatted.title, formatted.year, formatted.typeStr);
