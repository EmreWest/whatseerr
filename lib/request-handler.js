  ...
  try {
    ...
    if (res.status === 201 || res.status === 200) {
      try {
        ...
        logger?.debug('Subscribed user to TV show notifications', { 
          mediaId: tvShow.id, 
          is4k, 
          chatId 
        });
...
export async function handleTvShowSelection(cfg, jellyseerrClient, wahaClient, chatId, chosen, logger, is4k = false) {
  ...
  try {
    ...
    if (seasons.length === 0) {
      ...
      if (res.status === 201 || res.status === 200) {
        try {
          ...
          logger?.debug('Subscribed user to TV show notifications (all seasons)', { 
            mediaId: chosen.id, 
            is4k, 
            chatId 
          });
    ...
    // Note: The pending selection is stored in selection-command.js AFTER this function returns
    // This ensures the message is sent before state is updated, preventing race conditions
  ...
  try {
    ...
    if (res.status === 201 || res.status === 200) {
      try {
        ...
        subscriptionManager.addSubscription(chosen.id, mediaType, is4k, chatId);
        logger?.debug('Subscribed user to movie notifications', { 
          mediaId: chosen.id, 
          is4k, 
          chatId 
        });
