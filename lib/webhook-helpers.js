...
async function formatUserNotificationMessage(seerrData, jellyseerrClient, cfg, logger) {
  ...
  if (notificationType === 'MEDIA_AVAILABLE' && seerrData.media?.tmdbId && seerrData.media?.media_type) {
...
export function formatIssueCreatedMessageForAdmin(seerrData) {
  ...
  const issueId = seerrData.issue?.id || '';
...
export async function handleSeerrWebhook(cfg, jellyseerrClient, wahaClient, seerrData, logger) {
  ...
  async function sendNotificationToAdmin(notificationType, messageFormatter, seerrData, logger) {
    ...
    try {
      ...
      logger?.info(`✅ ${notificationType} notification sent to admin`, {
        notificationType,
        adminPhoneNumber,
        adminLidChatId,
        subject: seerrData.subject
  ...
  // Send admin notifications for types that require admin action/awareness
  if (seerrData.notification_type === 'MEDIA_PENDING' && seerrData.request?.request_id) {
    const requestId = String(seerrData.request.request_id);
    logger?.info('MEDIA_PENDING notification received, sending to admin', {
      requestId,
      subject: seerrData.subject,
      requestedBy: seerrData.request.requestedBy_username,
      mediaType: seerrData.media?.media_type
    });
    await sendNotificationToAdmin('MEDIA_PENDING', formatPendingRequestMessageForAdmin, seerrData, logger);
  } else if (seerrData.notification_type === 'MEDIA_FAILED') {
    logger?.info('MEDIA_FAILED notification received, sending to admin', {
      subject: seerrData.subject,
      requestedBy: seerrData.request?.requestedBy_username,
      mediaType: seerrData.media?.media_type
    });
    await sendNotificationToAdmin('MEDIA_FAILED', formatFailedMessageForAdmin, seerrData, logger);
  } else if (seerrData.notification_type === 'ISSUE_CREATED') {
    logger?.info('ISSUE_CREATED notification received, sending to admin', {
      subject: seerrData.subject,
      reportedBy: seerrData.issue?.reportedBy_username,
      issueId: seerrData.issue?.id
    });
    await sendNotificationToAdmin('ISSUE_CREATED', formatIssueCreatedMessageForAdmin, seerrData, logger);
  }

  // Extract email from notification data to identify the requester/user
  ...
  let requesterLidChatId = null; // Track requester LID to exclude from subscriber notifications
  let subscribersNotified = false; // Track if subscribers have been notified
  
  // Send notifications to users (MEDIA_PENDING, MEDIA_FAILED, and ISSUE_CREATED go to both admin and user)
  if (email) {
    ...
    if (userId) {
      // Save email mapping if not already in config
      const config = readConfigFile(logger);
      const wasInFile = config?.mappings?.emailMappings?.[email] !== undefined;
      ...
      if (phoneNumber) {
        ...
        if (lidChatId) {
          ...
          if (!isConfigured) {
            ...
            logger?.warn('Skipping notification to non-configured phone number', { 
              phoneNumber,
              lidChatId,
              userId,
              ...
              adminMatches: adminPhoneNumber === phoneNumber
            });
          ...
          // Check if recipient is admin - if so, skip user notification (admin already received it)
          ...
          if (isAdmin) {
            ...
            // Store LID for subscriber exclusion
            requesterLidChatId = lidChatId;
            return;
          }

          // Format notification message for user (non-admin)
          ...
          try {
            ...
            logger?.info(`✅ Notification sent to user ${phoneNumber} (userId: ${userId}, LID: ${lidChatId})`);
          } catch (err) {
  ...
  // Notify subscribers for MEDIA_AVAILABLE if not already notified
  // This handles cases where the notification doesn't have an email but media is available
  if (!subscribersNotified && seerrData.notification_type === 'MEDIA_AVAILABLE' && seerrData.media?.tmdbId && seerrData.media?.media_type) {
