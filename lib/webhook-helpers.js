...
export async function handleReaction(cfg, jellyseerrClient, wahaClient, webhookData) {
  ...
  if (!approvalInfo || !approvalInfo.requestId) {
    logger?.info('⏭️ Reaction not for approval message, ignoring', { messageId, messageBody: message.body?.substring(0, 100) });
    return;
  }
...
export async function handleSeerrWebhook(cfg, jellyseerrClient, wahaClient, seerrData, logger) {
  ...
  // Admin notification configuration - only action-required types that need admin attention
  // User-only notifications (MEDIA_AVAILABLE, MEDIA_APPROVED, etc.) are excluded
  const adminFormatters = {
  ...
  // Send admin notification for test notifications or listed types (MEDIA_PENDING requires request_id)
  const isTestNotification = notificationType === 'TEST_NOTIFICATION';
  const adminFormatter = adminFormatters[notificationType];
  const shouldNotifyAdmin = isTestNotification ||
    (adminFormatter && (notificationType !== 'MEDIA_PENDING' || seerrData.request?.request_id));

  if (shouldNotifyAdmin) {
    const formatter = adminFormatter || formatGenericNotification;
      ...
      if (phoneNumber) {
        ...
        if (lidChatId) {
          ...
          try {
            ...
            logger?.info(`📤 Sent notification to user${subject !== 'Unknown' ? `: "${subject}"` : ''}`);
          } catch (err) {
