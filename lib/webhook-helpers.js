...
import { sendMessage, sendImage, getMessageById, ensureLidFormatForMessaging } from './waha-client.js';
...
export function formatNotificationMessage({ notificationType, event, subject, extra = [], availableSeasons = null, isMovie = false }) {
  ...
  if (subject && subject.trim()) {
    const mediaEmoji = isMovie ? t('media.movie') : t('media.tv');
  ...
  if (!isMovie && availableSeasons !== null && Array.isArray(availableSeasons) && notificationType === 'MEDIA_AVAILABLE') {
    if (availableSeasons.length > 0) {
      if (availableSeasons.length === 1) {
        message += t('media.available_single_season', availableSeasons[0]);
      } else if (availableSeasons.length <= 5) {
        message += t('media.available_multiple_seasons', availableSeasons);
      } else {
        message += t('media.available_many_seasons', availableSeasons.length);
      }
    } else {
      message += t('media.available_show');
    }
    ...
    if (extra && Array.isArray(extra)) {
      ...
      if (requestedSeasonsEntry && requestedSeasonsEntry.value) {
        message += `\n${t('webhook.requested_seasons', requestedSeasonsEntry.value)}`;
      }
    }
  } else if (isMovie && notificationType === 'MEDIA_AVAILABLE') {
    message += t('media.enjoy_movie');
  }
...
async function formatAvailableNotificationMessage(seerrData, jellyseerrClient, cfg, logger) {
  ...
  const eventTitle = isMovie
    ? t('media.available_movie_title')
    : t('media.available_tv_title');
    ...
    try {
      ...
      for (const subscriberChatId of allSubscriberChatIds) {
        ...
        try {
          ...
          if (seerrData.image) {
            await sendImage(wahaClient, cfg, subscriberChatId, seerrData.image);
          }
 ...
 */
function buildAdminSection(baseMessage, lines) {
  let section = `\n\n━━━ ${t('webhook.admin_section_title')} ━━━\n`;
  section += lines.join('\n');
 ...
 */
function formatIssueAdminInfo(baseMessage, seerrData, actionBy, actionLabelKey, description) {
  const issueId = getIssueId(seerrData);
  const lines = [t('webhook.issue_action_by', t(actionLabelKey), actionBy)];

  if (issueId) {
    lines.push(t('webhook.issue_id', issueId));
  }

  lines.push(description);
  return buildAdminSection(baseMessage, lines);
}
 ...
 */
function formatIssueAdminInfoWithDetails(baseMessage, seerrData, actionBy, actionLabelKey, description) {
  const issueId = getIssueId(seerrData);
  ...
  if (issueType) {
    lines.push(t('webhook.issue_type_admin', formatIssueType(issueType)));
  }

  // Add user's message if provided
  if (userMessage && userMessage.trim()) {
    lines.push(t('webhook.issue_message_label', userMessage));
  }
...
async function appendAdminInfo(baseMessage, seerrData, jellyseerrClient, cfg, logger) {
  ...
  if (notificationType === 'MEDIA_PENDING') {
    ...
    if (!requestId || requestId === '') {
      return `${baseMessage}\n\n${t('webhook.requestNotFound')}`;
    }
    
    return buildAdminSection(baseMessage, [
      t('webhook.requested_by', requestedBy),
      t('webhook.request_id', requestId),
      '',
      t('webhook.approve', requestId),
      t('webhook.decline', requestId),
      t('webhook.cancel_hint')
    ]);
  }
  ...
  if (notificationType === 'MEDIA_FAILED') {
    ...
    const lines = [
      t('webhook.requested_by', requestedBy),
      isMovie ? t('webhook.failed_add_radarr') : t('webhook.failed_add_sonarr')
    ];

    if (seerrData.message) {
      lines.push('', t('webhook.failed_error_detail', seerrData.message));
    }
  ...
  if (notificationType === 'MEDIA_APPROVED' || notificationType === 'MEDIA_DECLINED') {
    const { requestedBy, requestId } = extractRequestData(seerrData);
    const lines = [t('webhook.requested_by', requestedBy)];

    if (requestId) {
      lines.push(t('webhook.request_id', requestId));
    }
    ...
    lines.push(notificationType === 'MEDIA_APPROVED'
      ? t('webhook.approved_by', modifiedBy)
      : t('webhook.declined_by', modifiedBy));
  ...
  if (notificationType === 'ISSUE_CREATED') {
    ...
    return formatIssueAdminInfoWithDetails(baseMessage, seerrData, reportedBy, 'webhook.issue_action_reported', t('webhook.issue_desc_created'));
  }
  ...
  if (notificationType === 'ISSUE_COMMENT') {
    ...
    if (issueType) {
      descLines.push(t('webhook.issue_type_admin', formatIssueType(issueType)));
    }

    if (commentMessage && commentMessage.trim()) {
      descLines.push(t('webhook.issue_comment_label', commentMessage));
    }

    descLines.push(t('webhook.issue_desc_comment'));

    return formatIssueAdminInfo(baseMessage, seerrData, commentedBy, 'webhook.issue_action_commented', descLines.join('\n'));
  }

  // ISSUE_RESOLVED
  if (notificationType === 'ISSUE_RESOLVED') {
    const resolvedBy = seerrData.issue?.resolvedBy_username || 'Unknown User';
    return formatIssueAdminInfoWithDetails(baseMessage, seerrData, resolvedBy, 'webhook.issue_action_resolved', t('webhook.issue_desc_resolved'));
  }

  // ISSUE_REOPENED
  if (notificationType === 'ISSUE_REOPENED') {
    const reopenedBy = seerrData.issue?.reportedBy_username || 'Unknown User';
    return formatIssueAdminInfoWithDetails(baseMessage, seerrData, reopenedBy, 'webhook.issue_action_reopened', t('webhook.issue_desc_reopened'));
  }
...
function formatPendingRequestMessageForUser(seerrData) {
  const { isMovie, mediaEmoji } = getMediaTypeAndEmoji(seerrData);
  const subject = getSubject(seerrData);
  const mediaTypeName = isMovie ? t('media.type_movie') : t('media.type_tv');

  let message = `⏳ ${t('webhook.pending_title')}\n\n`;
  message += `${mediaEmoji} ${subject}\n`;
  message += `${t('webhook.pending_type', mediaTypeName)}\n`;
  ...
  if (seerrData.media?.media_type !== 'movie' && seerrData.extra && Array.isArray(seerrData.extra)) {
    ...
    if (requestedSeasons?.value) {
      message += `${t('webhook.pending_seasons', mediaEmoji, requestedSeasons.value)}\n`;
    }
  }

  message += `\n${t('webhook.pending_status')}`;

  return message.trimEnd();
}

/**
 * Formats an automatically approved request message for user
 */
function formatAutoApprovedMessageForUser(seerrData) {
  const { isMovie } = getMediaTypeAndEmoji(seerrData);
  const subject = getSubject(seerrData);
  ...
  const messageKey = isMovie ? 'webhook.auto_approved_movie' : 'webhook.auto_approved_tv';
  return t(messageKey, title).trimEnd();
}

/**
 * Formats an approved request message for user (manual or auto via MEDIA_APPROVED type)
 */
function formatApprovedMessageForUser(seerrData) {
  const { isMovie } = getMediaTypeAndEmoji(seerrData);
  const subject = getSubject(seerrData);
  ...
  // Jellyseerr sends MEDIA_APPROVED for both manual and auto-approvals in some versions.
  // Check event text to pick the right translation.
  const event = seerrData.event || '';
  const isAuto = event.toLowerCase().includes('automatically');
  const messageKey = isAuto
    ? (isMovie ? 'webhook.auto_approved_movie' : 'webhook.auto_approved_tv')
    : (isMovie ? 'webhook.approved_movie' : 'webhook.approved_tv');
  return t(messageKey, title).trimEnd();
}
...
function formatFailedMessageForUser(seerrData) {
  ...
  let message = `${t('webhook.processingFailed')}\n\n`;
  message += `${mediaEmoji} ${subject}\n`;
  message += `${t('webhook.failed_type', mediaType)}\n\n`;
  message += t('webhook.failed_body');
...
function addIssueDetails(message, seerrData) {
  ...
  if (issueType) {
    ...
    message += `${t('webhook.issue_type_display', typeEmoji, typeText)}\n`;
  }

  // Add user's message if provided
  if (userMessage && userMessage.trim()) {
    message += `${t('webhook.issue_message_label', userMessage)}\n`;
  }
...
function formatIssueCommentMessageForUser(seerrData) {
  ...
  if (commentMessage && commentMessage.trim()) {
    message += `${t('webhook.issue_comment_label', commentMessage)}\n`;
  }

  message += `\n${t('webhook.issue_comment_body')}`;
...
const USER_NOTIFICATION_FORMATTERS = {
  ...
  'MEDIA_AUTO_APPROVED': formatAutoApprovedMessageForUser,
  'MEDIA_APPROVED': formatApprovedMessageForUser,
...
export function parseApprovalMessage(messageBody) {
  ...
  // Check for admin info section (only admins get this) — match translated title
  const adminTitle = t('webhook.admin_section_title');
  if (!messageBody.includes(`━━━ ${adminTitle} ━━━`)) {
    return null;
  }

  // Extract request ID — emoji-anchored so it works in any language
  const requestIdMatch = messageBody.match(/🆔[^:]*:\s*(\d+)/);
  ...
  // Extract requested by — emoji-anchored so it works in any language
  const requestedByMatch = messageBody.match(/👤[^:]*:\s*(.+?)(?:\n|$)/);
 ...
 */
async function sendToAllAdmins(cfg, wahaClient, logger, message, context, excludeLidChatId = null, imageUrl = null) {
  ...
  for (const { phoneNumber, mapping } of adminMappings) {
    ...
    try {
      ...
      if (imageUrl) {
        await sendImage(wahaClient, cfg, lidChatId, imageUrl);
      }
...
export async function handleSeerrWebhook(cfg, jellyseerrClient, wahaClient, seerrData, logger) {
        ...
        if (!lidChatId) {
          ...
          if (isConfigured) {
            try {
              ...
              if (seerrData.image && ['MEDIA_PENDING', 'MEDIA_AUTO_APPROVED', 'MEDIA_APPROVED', 'MEDIA_AVAILABLE'].includes(notificationType)) {
                await sendImage(wahaClient, cfg, lidChatId, seerrData.image);
              }
            } catch (err) {
  ...
  if (isAdminRelevant) {
    await sendToAllAdmins(cfg, wahaClient, logger, adminMessage, 'admin notification', requesterIsAdmin ? requesterLidChatId : null, seerrData.image || null);
  }
