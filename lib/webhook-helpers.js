...
async function appendAdminInfo(baseMessage, seerrData, jellyseerrClient, cfg, logger) {
  ...
  if (notificationType === 'ISSUE_CREATED') {
    ...
    return formatIssueAdminInfo(baseMessage, seerrData, reportedBy, 'Reported', 'Please review and resolve the issue.');
  }
  ...
  if (notificationType === 'ISSUE_COMMENT') {
    ...
    return formatIssueAdminInfo(baseMessage, seerrData, commentedBy, 'Commented', 'New comment added to issue.');
  }
  
  // ISSUE_RESOLVED
  if (notificationType === 'ISSUE_RESOLVED') {
    const resolvedBy = seerrData.issue?.resolvedBy_username || 'Unknown User';
    return formatIssueAdminInfo(baseMessage, seerrData, resolvedBy, 'Resolved', 'Issue has been resolved.');
  }
  
  // ISSUE_REOPENED
  if (notificationType === 'ISSUE_REOPENED') {
    const reopenedBy = seerrData.issue?.reportedBy_username || 'Unknown User';
    return formatIssueAdminInfo(baseMessage, seerrData, reopenedBy, 'Reopened', 'Issue has been reopened.');
  }
