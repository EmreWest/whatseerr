...
import fs from 'fs';
import { getConfigPath, getUserIdFromEmail, getPhoneNumberFromUserId, isLidFormat, isAdminChatId, getLidFromPhoneNumber, isPhoneNumberConfigured, isUserIdAdmin, getAdminPhoneNumber } from './utils.js';
...
/**
 * Formats notification message based on notification type and whether recipient is admin
 * @param {Object} seerrData - Seerr webhook data
 * @param {Object} jellyseerrClient - Jellyseerr API client
 * @param {Object} cfg - Configuration
 * @param {Object} logger - Logger instance
 * @param {boolean} isAdmin - Whether the recipient is an admin (uses admin formatters for admin-relevant types)
 * @returns {Promise<string>} Formatted notification message
 */
async function formatNotificationMessageForRecipient(seerrData, jellyseerrClient, cfg, logger, isAdmin = false) {
  if (isMediaAvailableNotification(seerrData)) {
    // MEDIA_AVAILABLE is always user-formatted (no admin version)
  ...
  // If recipient is admin and notification type has admin formatter, use admin formatter
  if (isAdmin && ADMIN_NOTIFICATION_FORMATTERS[notificationType]) {
    return ADMIN_NOTIFICATION_FORMATTERS[notificationType](seerrData);
  }
  
  // Otherwise, use user formatter (or generic if no specific formatter exists)
...
/**
 * Admin formatters for admin-relevant notification types (shared constant)
 * Defined after formatter functions to ensure they're available
 */
const ADMIN_NOTIFICATION_FORMATTERS = {
  'MEDIA_PENDING': formatPendingRequestMessageForAdmin,
  'MEDIA_FAILED': formatFailedMessageForAdmin,
  'ISSUE_CREATED': formatIssueCreatedMessageForAdmin
};
...
export async function handleSeerrWebhook(cfg, jellyseerrClient, wahaClient, seerrData, logger) {
  ...
  // Helper function to get admin LID chat ID (extracted to reduce duplication)
  async function getAdminLidChatId(context = 'notification') {
    const adminPhoneNumber = getAdminPhoneNumber(cfg);
    if (!adminPhoneNumber) {
      logger?.warn(`No admin user found in userIdMappings, cannot ${context}`);
      return null;
    }

    const adminLidChatId = await convertPhoneToLid(wahaClient, cfg, adminPhoneNumber, logger, `admin ${context}`);
    if (!adminLidChatId) {
      logger?.warn(`Cannot ${context} - admin LID cannot be resolved`);
      return null;
    }

    return adminLidChatId;
  }
  ...
  async function sendNotificationToAdmin(notificationType, messageFormatter, seerrData, logger) {
    const adminLidChatId = await getAdminLidChatId(`send ${notificationType} notification`);
    if (!adminLidChatId) {
      return;
    }
    ...
    try {
      ...
      // subject is available from outer scope via closure
      logger?.info(`📤 Sent notification to admin${subject !== 'Unknown' ? `: "${subject}"` : ''}`);
    } catch (err) {
  ...
  // Helper function to notify admin about LID resolution failure
  async function notifyAdminAboutLidResolutionFailure(details) {
    const adminLidChatId = await getAdminLidChatId('notify about LID resolution failure');
    if (!adminLidChatId) {
      return;
    }

    const { mediaType, mediaEmoji } = getMediaTypeAndEmoji(seerrData);
    const notificationSubject = seerrData.subject || 'Unknown';
    
    // Get username from userIdMappings if available
    const phoneNumberForLookup = details.phoneNumber;
    const mappings = cfg.mappings?.userIdMappings || {};
    const userMapping = phoneNumberForLookup ? mappings[phoneNumberForLookup] : null;
    ...
    try {
      await sendMessage(wahaClient, cfg, adminLidChatId, message);
      logger?.info('📤 Sent LID resolution failure notification to admin', {
      ...
      logger?.error('Failed to send LID resolution failure notification to admin', {
        ...getErrorDetails(err, 'notifyAdminAboutLidResolutionFailure'),
        userId: details.userId,
        phoneNumber: details.phoneNumber
      });
    }
  ...
  // Determine if notification type requires admin notification (when requester is NOT admin)
  const isTestNotification = notificationType === 'TEST_NOTIFICATION';
  const hasAdminFormatter = !!ADMIN_NOTIFICATION_FORMATTERS[notificationType];
  const requiresAdminNotification = isTestNotification ||
    (hasAdminFormatter && (notificationType !== 'MEDIA_PENDING' || seerrData.request?.request_id));

  // Helper to send admin notification with appropriate formatter
  const sendAdminNotificationIfNeeded = async () => {
    if (requiresAdminNotification) {
      const formatter = ADMIN_NOTIFICATION_FORMATTERS[notificationType] || formatGenericNotification;
      await sendNotificationToAdmin(notificationType, formatter, seerrData, logger);
    }
  };
  ...
  if (email) {
    ...
    if (userId) {
      ...
      // Check if requester has admin flag in userIdMappings
      const requesterIsAdmin = isUserIdAdmin(cfg, userId);
      ...
      if (phoneNumber) {
        ...
        if (!lidChatId) {
          // LID resolution failed - user hasn't initiated a chat with bot yet
          // Send notification to admin with details
          await notifyAdminAboutLidResolutionFailure({
            userId,
            email,
            phoneNumber,
            notificationType,
        ...
        // LID resolution succeeded - proceed with notification
        // Check if phone number is configured - only send to configured users
        ...
        // Format message based on whether requester is admin (admin gets admin-formatted messages for admin-relevant types)
        const notificationMessage = await formatNotificationMessageForRecipient(seerrData, jellyseerrClient, cfg, logger, requesterIsAdmin);

        try {
          await sendMessage(wahaClient, cfg, lidChatId, notificationMessage);
          logger?.info(`📤 Sent notification to ${requesterIsAdmin ? 'admin' : 'user'}${subject !== 'Unknown' ? `: "${subject}"` : ''}`);
        } catch (err) {
      ...
      // If requester is NOT admin and notification type requires admin notification, send separate admin notification
      if (!requesterIsAdmin) {
        await sendAdminNotificationIfNeeded();
      }
    } else {
      logger?.info(`⏭️ User not found - notification skipped`);
      // If no user found but notification type requires admin notification, send to admin anyway
      await sendAdminNotificationIfNeeded();
    }
  } else {
    logger?.debug('No email found in Seerr notification data', { 
      notificationType,
      hasNotifyUser: !!seerrData.notifyuser,
      hasRequest: !!seerrData.request,
      hasIssue: !!seerrData.issue
    });
    // If no email (e.g., TEST_NOTIFICATION) but notification type requires admin notification, send to admin
    await sendAdminNotificationIfNeeded();
  }
