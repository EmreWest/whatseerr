...
function migrateToNewFormat(key, chatIds, cfg = null) {
  ...
  const subscribers = chatIds.map(lid => {
    ...
    if (cfg?.lidMappings) {
      ...
      for (const [phoneChatId, mappedLid] of Object.entries(cfg.lidMappings)) {
        if (mappedLid === lid) {
          ...
          if (phoneNumber) {
            ...
            const userIdMappings = cfg.userIdMappings || {};
...
class SubscriptionManager {
   ...
   */
  addSubscription(mediaId, mediaType, is4k, chatId, title = '') {
    if (!mediaId || !chatId) {
    ...
    if (this.cfg?.lidMappings) {
      // Reverse lookup: find phoneChatId that maps to this lid
      for (const [phoneChatId, mappedLid] of Object.entries(this.cfg.lidMappings)) {
        if (mappedLid === chatId) {
          const phoneNumber = extractPhoneNumber(phoneChatId);
          if (phoneNumber) {
            subscriber.phoneNumber = phoneNumber;
            ...
            // Get userId and username from userIdMappings
            const userIdMappings = this.cfg.userIdMappings || {};
            const userMapping = userIdMappings[phoneNumber];
            if (userMapping) {
              subscriber.userId = userMapping.userId;
              subscriber.username = userMapping.username || '';
   ...
   * @param {string} chatId - User's chat ID (LID format)
   * @returns {Array} Array of subscription objects { mediaId, mediaType, is4k, title }
   */
  getUserSubscriptions(chatId) {
    ...
    for (const [key, entry] of Object.entries(this.subscriptions)) {
      ...
      if (chatIds.includes(chatId)) {
        ...
        // Add title if available (new format)
        if (!isOldFormat(entry) && entry.title) {
  ...
  /**
   * Updates the title for a subscription
   * @param {number} mediaId - Media ID (TMDB ID)
   * @param {string|number} mediaType - Media type ('movie'|'tv' or 1|2)
   * @param {boolean} is4k - Whether this is a 4K subscription
   * @param {string} title - Media title
   * @returns {boolean} True if title was updated
   */
  updateTitle(mediaId, mediaType, is4k, title) {
    if (!mediaId || !title) {
      return false;
    }
    ...
    if (this.subscriptions[key].title !== title) {
      this.subscriptions[key].title = title;
      this.saveSubscriptions();
      this.logger?.debug('Updated subscription title', { key, title });
    }
    return true;
  }
