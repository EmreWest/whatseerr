...
export function parseSeasonSelection(input, maxSeasons) {
  ...
  if (numbers.length === 0) {
    return { seasons: null, isAll: false, cancelled: false, error: '❌ Invalid format. Use numbers (e.g., "1" or "1,2,3") or "all"' };
  }
  ...
  if (filteredNumbers.length === 0) {
    return { seasons: null, isAll: false, cancelled: false, error: '❌ Season 0 (Specials) cannot be requested' };
  }
  
  // Validate all numbers are within range (1 to maxSeasons)
  const invalid = filteredNumbers.filter(n => n < 1 || n > maxSeasons);
  if (invalid.length > 0) {
    return { seasons: null, isAll: false, cancelled: false, error: `❌ Invalid season numbers: ${invalid.join(', ')}. Valid range: 1-${maxSeasons}` };
  }
...
export function formatSeasons(seasons, mediaDetails = null, is4k = false) {
  ...
  const statusPrefix = showStatus ? `📺 Show Status: ${showStatus}\n\n` : '';
  
  let message = `${statusPrefix}📺 Select seasons:\n`;
