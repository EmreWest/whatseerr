 ...
 */
export function getMovieMessage({ movieTitle, status, isRequested, isAvailable, hasNotification = false }) {
  if (isAvailable) {
  ...
  if (isRequested && !isAvailable) {
    // Already requested
    if (hasNotification) {
      return `This movie has already been requested and you will be notified when it becomes available.`;
    } else {
      return `This movie has already been requested, you can click on the notify me button directly under this message to be notified when it becomes available.`;
    }
  }
...
export function getSeasonMessage({ seasonNumber, tvShowTitle, isRequested, isAvailable, hasNotification = false }) {
  ...
  if (isRequested && !isAvailable) {
    ...
    if (hasNotification) {
      ...
      return `**Season ${seasonNumber}** has already been requested, you can click on the notify me button directly under this message to be notified when it becomes available.`;
    }
  }
...
export function getAllSeasonsMessage({ tvShowTitle, canBeRequested }) {
  ...
  return `Some seasons of **${tvShowTitle}** are already requested or available.`;
}
...
export function getFutureSeasonsMessage({ 
  ...
  if (allSeasonsAvailable) {
    if (hasNotification) {
      ...
      return `**All seasons** are already available, you can click on the notify me button directly under this message to be notified when future seasons becomes available.`;
    }
  }
  ...
  if (allSeasonsRequested) {
    if (hasNotification) {
      ...
      return `**All seasons** have been already requested, you can click on the notify me button directly under this message to be notified when future seasons becomes available.`;
    }
  }
  ...
  if (hasNotification) {
    ...
    return `**Future seasons** have already been requested, you can click on the notify me button directly under this message to be notified when future seasons becomes available.`;
  }
}
...
export function getShowCannotBeRequestedMessage() {
  return `This show cannot be automatically requested, please ask the server owner to manually add it.`;
}
...
export function hasNotificationSetUp(mediaDetails, userId = null) {
  // TODO: Implement notification checking via Overseerr API
