...
export async function createRequest(client, cfg, media, seasons = null, is4k = false, logger = null, chatId = null) {
  const headers = {
    'X-Api-Key': cfg.jellyseerr.apiKey,
  };
  ...
  const userId = getUserIdFromChatId(cfg, chatId);
  if (userId) {
  ...
  logger?.debug('Creating request', { path: 'request', body, is4k, chatId, userId });
