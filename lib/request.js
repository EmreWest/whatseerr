...
export async function searchTitle(client, cfg, query, type, year, logger = null) {
  ...
  logger?.debug('Searching title', { query, encodedQuery, type, year });
  const headers = {
  ...
  if (res.status !== 200) {
    logger?.debug('Search API error response', {
      status: res.status,
      query: encodedQuery,
      type,
      year
    });
    logger?.trace('Search API error response body', {
      status: res.status,
      query: encodedQuery,
      data: res.data
    });
  ...
  logger?.trace('Search API full response', {
    status: res.status,
    query: encodedQuery,
    data: res.data
  });
...
export async function getMediaDetails(client, cfg, mediaId, mediaType, logger = null) {
  ...
  if (res.status !== 200) {
    logger?.debug('Media details API error response', {
      status: res.status,
      mediaType: typePath,
      mediaId,
      endpoint: `${typePath}/${mediaId}`
    });
    logger?.trace('Media details API error response body', {
      status: res.status,
      mediaType: typePath,
      mediaId,
      data: res.data
  ...
  logger?.debug('Media details API response received', {
    status: res.status,
    mediaType: typePath,
    mediaId,
    hasMediaInfo: !!res.data?.mediaInfo,
    hasExternalIds: !!res.data?.externalIds
  });
  logger?.trace('Media details API full response', {
    status: res.status,
    mediaType: typePath,
    mediaId,
    data: res.data
...
export async function createRequest(client, cfg, media, seasons = null, is4k = false, logger = null, chatId = null, wahaClient = null) {
  ...
  // Per Overseerr API spec: seasons can be array of numbers OR string "all"
  // API reference: seasons: oneOf: [array of numbers (minimum: 0), string enum: ["all"]]
  // Note: API allows season 0 (Specials), but we filter it out in parseSeasonSelection() as business logic
  if (seasons) {
    if (seasons === 'all') {
      body.seasons = 'all';
    } else if (Array.isArray(seasons) && seasons.length > 0) {
      // Array is already filtered to exclude season 0 by parseSeasonSelection()
      body.seasons = seasons;
    }
  }

  // Get userId from mappings based on chatId
  // Note: This should only be called for configured phone numbers (verified via isPhoneNumberConfigured)
  ...
  // Always send X-API-User header (use defaultUserId as fallback)
  const effectiveUserId = userId || cfg.jellyseerr.defaultUserId;
  headers['X-API-User'] = String(effectiveUserId);

  logger?.debug('Using userId in X-API-User header', {
    userId: effectiveUserId,
    chatId,
    source: userId ? 'resolved' : 'defaultUserId'
  });
  ...
  logger?.trace('Create request API body', { body, headers: { ...headers, 'X-Api-Key': '[REDACTED]' } });
  ...
  logger?.debug('Create request API response received', {
    status: res.status,
    mediaId: media.id,
    mediaType,
  ...
  logger?.trace('Create request API full response', {
    status: res.status,
    mediaId: media.id,
    mediaType,
    data: res.data
 ...
 * @param {number|null} userId - User ID to use in X-API-User header (falls back to defaultUserId)
 * @returns {Promise<Object>} API response
 */
export async function approveRequest(client, cfg, requestId, logger = null, userId = null) {
  const requestIdStr = String(requestId);
  const effectiveUserId = userId || cfg.jellyseerr.defaultUserId;

  const headers = {
    'X-Api-Key': cfg.jellyseerr.apiKey,
    'X-API-User': String(effectiveUserId)
  };

  logger?.debug('Approving request with userId', {
    userId: effectiveUserId,
    requestId: requestIdStr,
    source: userId ? 'provided' : 'defaultUserId',
    endpoint: `request/${requestIdStr}/approve`
  });
  ...
  if (res.status !== 200) {
    logger?.debug('Approve request API error response', {
      status: res.status,
      requestId: requestIdStr
    });
    logger?.trace('Approve request API error response body', {
      status: res.status,
      requestId: requestIdStr,
      data: res.data
    });
  ...
  logger?.debug('Approve request API response received', {
    status: res.status,
    requestId: requestIdStr
  });
  logger?.trace('Approve request API full response', {
    status: res.status,
    requestId: requestIdStr,
    data: res.data
  });
 ...
 * @returns {Promise<Object>} API response
 */
export async function declineRequest(client, cfg, requestId, logger = null, userId = null) {
  const requestIdStr = String(requestId);
  const effectiveUserId = userId || cfg.jellyseerr.defaultUserId;
  ...
  logger?.debug('Declining request with userId', {
    userId: effectiveUserId,
    requestId: requestIdStr,
    source: userId ? 'provided' : 'defaultUserId',
    endpoint: `request/${requestIdStr}/decline`
  });
  ...
  if (res.status !== 200) {
    logger?.debug('Decline request API error response', {
      status: res.status,
      requestId: requestIdStr
    });
    logger?.trace('Decline request API error response body', {
      status: res.status,
      requestId: requestIdStr,
      data: res.data
    });
  ...
  logger?.debug('Decline request API response received', {
    status: res.status,
    requestId: requestIdStr
  });
  logger?.trace('Decline request API full response', {
    status: res.status,
    requestId: requestIdStr,
    data: res.data
  });
 ...
 * @param {Object} client - HTTP client
 * @param {Object} cfg - Configuration object
 * @param {string|number} requestId - Request ID to fetch
 * @param {Object} logger - Logger instance
 * @returns {Promise<Object|null>} Request details or null if not found
 */
export async function getRequest(client, cfg, requestId, logger = null) {
  const headers = {
    'X-Api-Key': cfg.jellyseerr.apiKey,
  };

  const requestIdStr = String(requestId);
  logger?.debug('Fetching request details', { requestId: requestIdStr });

  try {
    const res = await client.request('GET', `request/${requestIdStr}`, {
      headers,
    });

    if (res.status === 404) {
      logger?.debug('Request not found', { requestId: requestIdStr });
      return null;
    }

    if (res.status !== 200) {
      logger?.debug('Get request API error response', {
        status: res.status,
        requestId: requestIdStr
      });
      logger?.trace('Get request API error response body', {
        status: res.status,
        requestId: requestIdStr,
        data: res.data
      });
      const bodySnippet = typeof res.data === 'string'
        ? res.data.slice(0, 500)
        : JSON.stringify(res.data).slice(0, 500);
      const err = new Error(`Get request failed with status ${res.status}: ${bodySnippet}`);
      err.response = res;
      throw err;
    }

    logger?.debug('Get request API response received', {
      status: res.status,
      requestId: requestIdStr,
      hasModifiedBy: !!res.data?.modifiedBy,
      data: res.data  // Full response data (filtered by apiResponse flag)
    });

    return res.data;
  } catch (err) {
    // If it's a 404, return null (request doesn't exist)
    if (err.response?.status === 404) {
      logger?.debug('Request not found (404)', { requestId: requestIdStr });
      return null;
    }
...
/**
 * Formats quality text suffix for logging/messages
 * @param {boolean} is4k - Whether this is a 4K request
 * @returns {string} Quality text suffix (e.g., ' (4K)' or '')
 */
export function formatQualityText(is4k) {
  return is4k ? ' (4K)' : '';
}

/**
 * Converts media type to display string
 * @param {string|number} mediaType - Media type ('movie'|'tv' or 1|2)
 * @returns {string} Display string ('Movie' or 'TV Show')
 */
export function formatMediaType(mediaType) {
  return (mediaType === 'movie' || mediaType === 1) ? 'Movie' : 'TV Show';
}
...
export function formatMedia(media) {
  ...
  const typeStr = formatMediaType(media.mediaType);
    ...
    while (true) {
      ...
      try {
        ...
        if (res.status === 201 || res.status === 200) {
          ...
          logger?.trace('409 conflict response details', res.data);
        } else {
          logger?.error(`Unexpected response status: ${res.status}`);
          logger?.trace('Unexpected response body', res.data);
        }
      } catch (err) {
