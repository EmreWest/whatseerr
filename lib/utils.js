 ...
 * @returns {Promise<boolean>} True if phone number is configured, false otherwise
 */
export async function isPhoneNumberConfigured(cfg, chatId, wahaClient = null) {
  ...
  if (mappings[phoneNumber] !== undefined) {
    ...
    if (!mapping || typeof mapping !== 'object' || typeof mapping.userId !== 'number') {
      logger?.debug('Invalid userIdMappings structure for phone number', { 
        phoneNumber, 
        mapping,
        originalChatId: chatId
      });
  ...
  logger?.debug('Phone number is not configured in userIdMappings or as admin', { 
    phoneNumber,
