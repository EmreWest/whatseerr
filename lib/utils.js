...
function normalizeConfig(rawCfg) {
  ...
  // Support both top-level baseUrl/apiKey and nested jellyseerr object formats
  const hasTopLevelKeys = typeof cfg.baseUrl === 'string' || typeof cfg.apiKey === 'string';

  // If jellyseerr object is missing, build it from top-level keys.
  if (!cfg.jellyseerr) {
    cfg.jellyseerr = {};
  }
...
export function loadConfig(options = {}) {
  ...
  try {
    ...
    if (requireWebhook) {
      if (!cfg.requests?.port || typeof cfg.requests.port !== 'number') {
        logger?.error('config.json must contain requests.port (number).');
        process.exit(1);
      }
      if (!cfg.requests?.path || typeof cfg.requests.path !== 'string') {
        logger?.error('config.json must contain requests.path (string).');
        process.exit(1);
      }
    }
...
export function getWebhookUrl(cfg, hostOverride = null) {
  ...
  const port = cfg.requests?.port;
  const webhookPath = cfg.requests?.path;
  if (!protocol || !host || !port || !webhookPath) {
    throw new Error('Missing webhook config: expected protocol, host, requests.port, requests.path in config.json');
  }
...
export function getPhoneNumberFromUserId(cfg, userId) {
  ...
  for (const [phoneNumber, mappedUserId] of Object.entries(mappings)) {
