...
function normalizeConfig(rawCfg) {
  ...
  if (cfg.system || cfg.services || cfg.mappings || cfg.commands) {
    // Extract services first to ensure jellyseerr object exists
    if (cfg.services) {
    ...
    if (cfg.system) {
      ...
      if (cfg.system.admin) {
        // Ensure jellyseerr object exists (may have been created above)
        if (!cfg.jellyseerr) {
          cfg.jellyseerr = {};
        }
        // Map system.admin to jellyseerr.adminDetails for code compatibility
        // Don't overwrite if it already exists (backward compatibility)
        if (!cfg.jellyseerr.adminDetails) {
    ...
    if (cfg.jellyseerr) {
      ...
      // Also support adminDetails for backward compatibility
      if (cfg.jellyseerr.adminDetails && !cfg.jellyseerr.admin) {
        cfg.jellyseerr.admin = cfg.jellyseerr.adminDetails;
      }
    }
