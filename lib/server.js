...
export async function createServer(cfg, jellyseerrClient, wahaClient, handleMessage) {
  ...
  fastify.post(seerrPath, async (request, reply) => {
    logger?.info('📥 Seerr webhook received', { url: request.url });
  ...
  fastify.post(requestsPath, async (request, reply) => {
    ...
    try {
      ...
      if (webhookData.event) {
        ...
        logger?.info('📥 Webhook event received', logData);
      } else {
      ...
      // Process message events - prefer 'message.any' to avoid duplicates
        ...
        if (messageId && !stateManager.isMessageProcessed(messageId)) {
          ...
          logger?.info('🔄 Duplicate webhook event - message already queued/processed, skipping', {
            messageId,
            from: webhookData.payload?.from,
            event: webhookData.event,
            reason: 'This message ID was already processed or queued (preventing duplicate webhook handling)'
          });
        }
      }
      ...
      logger?.error('Error processing WAHA webhook', getErrorDetails(error, 'processWAHAWebhook'));
      throw error; // Will be caught by error handler
    }
  });
