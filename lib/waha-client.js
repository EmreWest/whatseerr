...
export async function getPhoneNumberByLid(client, cfg, lid) {
  ...
  logger?.trace('Starting LID to phone number resolution', { lid });
  ...
  logger?.trace('Extracted LID number from LID format', { lid, lidNumber });
  ...
  logger?.debug('Resolving LID to phone number via WAHA API', { lid });
  logger?.trace('LID resolution API call details', { 
    lid, 
    lidNumber, 
    session, 
    url,
  ...
  try {
    ...
    logger?.debug('LID resolved to phone number', { 
      lid, 
      status: res.status,
      phoneNumber: res.data?.pn
    });
    logger?.trace('WAHA LID resolution API full response', { 
      lid, 
      status: res.status,
      data: res.data
    });
    ...
    logger?.warn('Exception during LID resolution', { 
      ...getErrorDetails(err, 'resolveLidToPhoneNumber'),
      lid
    });
    return null;
  }
...
export async function getLidByPhoneNumber(client, cfg, phoneChatId) {
  ...
  logger?.trace('Starting phone number to LID resolution', { phoneChatId });
  ...
  logger?.trace('Extracted phone number from phone format chatId', { phoneChatId, phoneNumber });
  ...
  logger?.debug('Resolving phone number to LID via WAHA API', { phoneChatId });
  logger?.trace('Phone-to-LID resolution API call details', { 
    phoneChatId, 
    phoneNumber, 
    session, 
    url,
  ...
  try {
    ...
    logger?.debug('Phone number resolved to LID', { 
      phoneChatId, 
      status: res.status,
      lid: res.data?.lid
    });
    logger?.trace('WAHA phone-to-LID resolution API full response', { 
      phoneChatId, 
      status: res.status,
      data: res.data
    });
    ...
    logger?.trace('Phone number successfully resolved to LID', { 
      phoneChatId, 
      lid,
      phoneNumber
    });
    ...
    logger?.warn('Exception during phone number to LID resolution', { 
      ...getErrorDetails(err, 'resolvePhoneNumberToLid'),
      phoneChatId
    });
    return null;
  }
...
async function sendChatAction(client, cfg, lidChatId, endpoint, actionName) {
  ...
  logger?.trace(`Sending WAHA ${actionName}`, { 
    chatId: lidChatId, 
    endpoint,
  ...
  logger?.trace(`WAHA ${actionName} response received`, { 
    status: res.status,
    chatId: lidChatId
  });
  logger?.trace(`WAHA ${actionName} full response`, { 
    status: res.status,
    chatId: lidChatId,
    data: res.data
  });
...
export async function ensureLidFormatForMessaging(client, cfg, chatId) {
  ...
  if (chatId.endsWith('@c.us')) {
    ...
    logger?.trace('LID not in config mapping, resolving via WAHA API', { phoneChatId: chatId });
    const lidFromApi = await getLidByPhoneNumber(client, cfg, chatId);
    
    if (!lidFromApi) {
      // This is expected when user hasn't initiated a chat with the bot yet
      logger?.warn('No LID found - user may not have initiated a chat with the bot yet', {
        phoneChatId: chatId,
    ...
    logger?.trace('Resolved phone number to LID via WAHA API', { 
      phoneChatId: chatId, 
...
export async function sendMessage(client, cfg, chatId, text, options = {}) {
  ...
  // Log full outgoing message at info level
  ...
  try {
    ...
    try {
      ...
      logger?.trace('Sent seen indicator', { lidChatId });
    } catch (err) {
      // Log error but continue - seen is best effort
      logger?.warn('Failed to send seen indicator', getErrorDetails(err, 'sendSeen'));
    }

    // Step 2: Start typing indicator (using LID format)
    try {
      await startTyping(client, cfg, lidChatId);
      logger?.trace('Started typing indicator', { lidChatId });
    } catch (err) {
      logger?.warn('Failed to start typing indicator', getErrorDetails(err, 'startTyping'));
    }
    ...
    const regularCharCount = countRegularTextChars(text);
    logger?.trace('Typing interval calculated', { 
    ...
    try {
      ...
      logger?.trace('Stopped typing indicator', { lidChatId });
    } catch (err) {
      logger?.warn('Failed to stop typing indicator', getErrorDetails(err, 'stopTyping'));
    }
    ...
    logger?.debug('Sending message via WAHA API', {
      lidChatId,
      textLength: text.length,
      hasReplyTo: !!replyTo
    });
    logger?.trace('WAHA sendText request details', {
    ...
    logger?.debug('WAHA sendText response received', {
      status: res.status,
      messageId: res.data?.id,
      lidChatId
    });
    logger?.trace('WAHA sendText full response', {
      status: res.status,
      lidChatId,
      data: res.data
    });
    ...
    logger?.error('Error in sendMessage flow', getErrorDetails(err, 'sendMessage'));
    throw err;
  }
}

/**
 * Sends an image to a WhatsApp chat (best-effort, does not throw)
 * @param {Object} client - WAHA HTTP client
 * @param {Object} cfg - Configuration object with waha settings
 * @param {string} chatId - WhatsApp chat ID
 * @param {string} imageUrl - Full URL of the image to send
 * @returns {Promise<void>}
 */
export async function sendImage(client, cfg, chatId, imageUrl) {
  if (!imageUrl) return;
  const logger = cfg.__logger;
  ...
  try {
    ...
    logger?.warn('sendImage: Failed to convert chatId to LID', getErrorDetails(err, 'sendImage'));
    return;
  }
  ...
  try {
    await client.request('POST', '/api/sendImage', { headers, body });
    logger?.debug('Cover image sent', { lidChatId });
  } catch (err) {
    logger?.warn('Failed to send cover image', getErrorDetails(err, 'sendImage'));
  }
}
...
export async function getMessageById(client, cfg, chatId, messageId) {
  ...
  logger?.debug('Fetching message by ID from WAHA API', { 
    chatId, 
    lidChatId, 
    messageId, 
    encodedMessageId, 
  ...
  try {
    ...
    logger?.warn('Exception fetching message by ID', {
      ...getErrorDetails(err, 'getMessageById'),
      chatId,
      messageId
    });
    return null;
