...
import { isLidFormat, ensureLidFormat, setLidMapping, extractPhoneNumber, getIdentifierType } from './utils.js';
 ...
 * LID format is required for messaging - this function expects LID format chatId
 * @param {Object} client - WAHA HTTP client
 * @param {Object} cfg - Configuration object with waha settings
 * @param {string} chatId - WhatsApp chat ID in LID format (e.g., "151169980723349@lid")
...
export async function sendSeen(client, cfg, chatId) {
  ...
  if (!chatId || !chatId.endsWith('@lid')) {
    throw new Error(`sendSeen requires LID format chatId, got: ${chatId}`);
  }
  ...
  // API returns 201 Created for successful seen indicators
  // Per API docs: SendSeenRequest requires chatId and session (messageId is deprecated, use messageIds array)
...
export async function startTyping(client, cfg, chatId) {
  ...
  // Ensure LID format
  if (!chatId || !chatId.endsWith('@lid')) {
    throw new Error(`startTyping requires LID format chatId, got: ${chatId}`);
  }
  ...
  // API returns 201 Created for successful typing indicator start
  // Per API docs: ChatRequest requires chatId and session
...
export async function stopTyping(client, cfg, chatId) {
  ...
  // Ensure LID format
  if (!chatId || !chatId.endsWith('@lid')) {
    throw new Error(`stopTyping requires LID format chatId, got: ${chatId}`);
  }
  ...
  // API returns 201 Created for successful typing indicator stop
  // Per API docs: ChatRequest requires chatId and session
 ...
 */
async function ensureLidFormatForMessaging(client, cfg, chatId) {
  const logger = cfg.__logger;
...
export async function sendMessage(client, cfg, chatId, text, options = {}) {
  ...
  try {
    ...
    // Step 3: Wait for random interval based on message size
    ...
    const identifierInfo = getIdentifierType(lidChatId);
    ...
    // API returns 201 Created for successful message sends
